...
**mode_of_work**: "Remote", "Hybrid", "Onsite", or "".
...
export async function extractWithOllamaSemanticAssistant(text: string): Promise<SemanticExtraction> {
  ...
  console.log(`[Ollama] Calling ${env.OLLAMA_MODEL} at ${env.OLLAMA_BASE_URL} — "${inputSnippet}..."`);
  ...
  try {
    const response = await axios.post(
      `${env.OLLAMA_BASE_URL}/api/chat`,
      {
        ...
        messages: [
          ...
          { role: "user", content: `Extract placement information from the following text:\n\n${stripEmailHeaders(text).slice(0, 6000)}` }
        ],
    ...
    console.log(`[Ollama] Response in ${(elapsedMs / 1000).toFixed(1)}s, length: ${rawOutput.length}`);
    const parsed = safeParseJsonBlock(rawOutput);
    console.log("[Ollama] Parsed:", JSON.stringify(parsed, null, 2));
    return parsed;
  } catch (err: unknown) {
    const elapsedMs = Date.now() - startMs;
